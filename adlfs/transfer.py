...
class ADLTransferClient(object):
    ...
    `fn(adlfs, src, dst, offset, size, blocksize, shutdown_event)`. `adlfs` is
    ...
    `src` to read `size` bytes from. `blocksize` is the number of bytes in a
    chunk to write at one time. The callable should return an integer
